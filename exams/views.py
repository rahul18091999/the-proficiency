...
def addNLEs(request):
    ...
    if request.method == "POST":
        ...
        if date and time and title and sp and desc:
            ...
            tid = {}
            if data:
                temp = database.child('notifications').child(
            ...
            if token:
                print(token)
                token.append(
                    'EfadsfadsfasdfasdxponentPushfasdTofsadfskfadssdafsdfen[4iSHToD76BENf7-ujv4hcN')
                import requests
        ...
        for i in l:
            ...
            if i['value']:
                for j in nledata.val()[i['value']]['mainly']:
                    ...
                    n.append(
                        {'value': j, 'name': mainlydata[j[:6]]['mainly'][j]['details']['name']})
            m.append(n)
    if request.method == "POST":
        ...
        if(ques != "" and opt1 != "" and opt2 != "" and opt3 != "" and opt4 != "" and NLE is not None and mainly is not None):
            print('Hello')
            free = database.child('exams').child('NLE').child(NLE).child(
                'mainly').child(mainly).child('questions').child('free').get().val()
            ...
            database.child('exams').child('NLE').child(NLE).child('mainly').child(
                mainly).child('questions').update({'free': free+1})
            data = {
                'question': "",
                'opt1': "",
                'opt2': "",
...
def viewNleQues(request):
    ...
    nle = d[8:]+d[5:7]+d[:4]
    data = database.child('exams').child(
        'NLE').child(nle).child('mainly').get()
    mainlydata = database.child('prepration').get().val()
...
def addAnsKey(request):
    ...
    count = database.child('exams').child('NLE').child(nle).child('mainly').child(
        mid).child('questions').child('free').shallow().get().val()
    if request.method == 'POST':
        ans = []
        ...
        for index, item in enumerate(ans):
            database.child('exams').child('NLE').child(nle).child('mainly').child(
                mid).child('questions').child(index+1).update({'optC': item})
...
def viewExamStudent(request):
    ...
    for i in range(len(dailyTimedate)):
        dailyTimedate[i] = dailyTimedate[i][4:]+'-' + \
            dailyTimedate[i][2:4]+'-'+dailyTimedate[i][:2]
    dailyTimedate.sort()
